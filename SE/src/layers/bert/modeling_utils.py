...
class PreTrainedModel(nn.Module):
    ...
    def _compute_sequential_entropy(batch_size,probs):
        entropy_score = scipy.stats.entropy(probs, axis = 2)
        entropy_score = entropy_score.view(batch_size, -1)
        return entropy_score.mean(-1)
    ...
    def _generate_beam_search(
        ...
        seq_total_entropy = torch.zeros((batch_size), dtype=torch.float, device=input_ids.device)
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(input_ids, past=past)
            outputs = self(**model_inputs)  # (batch_size * num_beams, cur_len, vocab_size)    
            probs = torch.exp(outputs[1])
            seq_total_entropy += self._compute_sequential_entropy(batch_size, probs)
        ...
        return decoded, logprobs, seq_total_entropy

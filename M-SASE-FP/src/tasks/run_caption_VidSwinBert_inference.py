...
from src.utils.filter_cluster_postprocessing import kmeans_filtering
import scipy.stats
import joblib
import glob
import operator

def filter_cands(sentences, scores):
    bc = BertClient()
    detokenize = MosesDetokenizer('en')
    candidates, scores = kmeans_filtering(
                candidates, scores, 30, True, bc, detokenize)
...
def inference(args, video_path, model, tokenizer, tensorizer):
    ...
    with torch.no_grad():
        ...
        return_conf = []
        for i in range(5):
            video_feats = torch.load(f'{video_path[:-4]}_video_feats_fp_{i}.bin')
            inputs = {'is_decode': True,
                'input_ids': data_sample[0][None,:], 'attention_mask': data_sample[1][None,:],
                'token_type_ids': data_sample[2][None,:], 'img_feats': video_feats,
            ...
            for caps, confs in zip(all_caps, all_confs):
                for cap, conf in zip(caps, confs):
                    ...
                    return_conf.append(conf)
        
        return_caps, return_conf = filter_cands(return_caps, return_conf)
        acquisation_score = scipy.stats.entropy(return_conf, axis=1)
        
    return return_caps, return_conf, acquisation_score
...
def update_existing_config_for_inference(args):
    ...
    train_args.url_video = args.url_video
    return train_args
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--process_folder', type=str, default='None')
    parser.add_argument('--url_video', type=str, default='None')
...
def main(args):
    ...
    files = glob.glob(f'{args.process_folder}/*mp4')
    acquisation_table = {}
    for file in files:
        _, _, acquisation_score = inference(args, file, vl_transformer, tokenizer, tensorizer)    
        acquisation_table[file] = acquisation_score
    
    sorted_dict = sorted(acquisation_table.items(), key = operator.itemgetter(1))
    joblib.dump(sorted_dict.keys()[:325], 'top_selected_samples.pkl')
